...
function operator(pro) {
  ...
  pro.forEach((e) => {
    if (e.name.includes("DMIT")) { e._skip = true; return; }
...
// prettier-ignore
function jxh(e) { const n = e.reduce((e, n) => { if (n._skip) { e.push({ name: n.name, count: 1, items: [n] }); return e; } const t = e.find((e) => e.name === n.name && !e._skip); if (t) { t.count++; t.items.push({ ...n, name: `${n.name}${XHFGF}${t.count.toString().padStart(2, "0")}`, }); } else { e.push({ name: n.name, count: 1, items: [{ ...n, name: `${n.name}${XHFGF}01` }], }); } return e; }, []);const t=(typeof Array.prototype.flatMap==='function'?n.flatMap((e) => e.items):n.reduce((acc, e) => acc.concat(e.items),[])); e.splice(0, e.length, ...t); return e;}
// prettier-ignore
